...
import { deleteSubmodule } from "../../api/submodules";
...
interface Props {
  open: boolean;
  onClose: () => void;
  submoduleToDelete: SubModule | null;
  fetchSubmodules: () => void;
}

const SubmoduleDeleteModal: React.FC<Props> = ({
  open,
  onClose,
  submoduleToDelete,
  fetchSubmodules,
  ...
  const handleDelete = async () => {
    if (submoduleToDelete) {
      await deleteSubmodule(submoduleToDelete.id);
      fetchSubmodules();
      onClose();
    }
  };
      ...
      <div className="flex items-center justify-center min-h-screen px-4">
        ...
        <div className="relative bg-white p-8 rounded-2xl w-full max-w-sm z-50 shadow-2xl">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-semibold text-gray-800">Delete Submodule</h2>
          ...
          <p className="text-gray-700 mb-6">
            ...
            <span className="font-bold text-red-500">{submoduleToDelete?.name}</span>?
          </p>
...
export default SubmoduleDeleteModal;
